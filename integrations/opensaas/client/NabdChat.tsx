 ...
 * 
 * في main.wasp أضف:
 * route NabdChatRoute { path: "/ai", to: NabdChatPage }
 * page NabdChatPage {
...
import { askNabd } from 'wasp/client/operations';
...
const AGENT_MODES = [
    { id: 'general', name: 'مساعد عام', icon: '🤖', color: 'cyan' },
    { id: 'coder', name: 'مبرمج', icon: '👨‍💻', color: 'green' },
    { id: 'writer', name: 'كاتب محتوى', icon: '📝', color: 'purple' },
    { id: 'researcher', name: 'باحث', icon: '🔍', color: 'orange' },
];

const MODEL_OPTIONS = [
    { id: 'llama-3.1-8b-instant', name: 'سريع 🚀', premium: false },
    { id: 'llama-3.3-70b-versatile', name: 'ذكي 🧠', premium: true },
];
        ...
        try {
            ...
            setMessages((prev) => [
                ...prev,
                {
                    id: crypto.randomUUID(),
                    role: 'error',
                    content: error.message || 'حدث خطأ غير متوقع',
                },
            ]);
        } finally {
            setIsLoading(false);
                ...
                <div className="flex gap-2">
                    {MODEL_OPTIONS.map((model) => (
                        <button
                            ...
                            disabled={model.premium} // Disable premium if not subscribed
                            className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${modelName === model.id
                                    ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/50'
                                    : 'bg-slate-800/50 text-slate-400 border border-slate-700 hover:border-slate-600'
                                } ${model.premium ? 'opacity-50 cursor-not-allowed' : ''}`}
                        >
                            {model.name}
                            {model.premium && <span className="mr-1">💎</span>}
                        </button>
                    ))}
                </div>
            </header>
            ...
            <div className="flex justify-center gap-2 p-4 border-b border-slate-700/30">
                {AGENT_MODES.map((agent) => (
                    <button
                        ...
                        className={`px-4 py-2 rounded-xl flex items-center gap-2 transition-all ${agentMode === agent.id
                        ...
                        <div className="flex flex-wrap gap-2 mt-6 justify-center">
                            {['ما هي آخر أخبار التكنولوجيا؟', 'اكتب لي كود بايثون', 'لخص هذا المقال'].map((prompt) => (
                                <button
                                    ...
                                    className="px-4 py-2 rounded-xl bg-slate-800/50 text-slate-300 text-sm border border-slate-700 hover:border-cyan-500/50 hover:bg-slate-800 transition-all"
                                >
                                    {prompt}
                                </button>
                            ))}
            ...
            <form onSubmit={handleSubmit} className="p-4 border-t border-slate-700/50 backdrop-blur-xl bg-slate-900/50">
                <div className="max-w-4xl mx-auto flex gap-3">
                    ...
                    <button
                        ...
                        className="px-6 py-3 bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-medium rounded-xl hover:from-cyan-400 hover:to-blue-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-cyan-500/25 flex items-center gap-2"
                    >
                        <span>إرسال</span>
